...
extern Memb_func* memb_func;
 ...
 "evaluate_fct_it", _hoc_evaluate_fct,
 0, 0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 15, 5);
 	...
 	ivoc_help("help ?1 it d:/Dropbox/neuro_sim_lab/netpyne_scripts/my_knox2/IT.mod\n");
...
static int  evaluate_fct ( _threadargsprotocomma_ double _lv ) {
 ...
 _lVm = _lv + shift ;
   ...
   tau_h = 30.8 + ( 211.4 + exp ( ( _lVm + 113.2 ) / 5.0 ) ) / ( 1.0 + exp ( ( _lVm + 84.0 ) / 3.2 ) ) ;
   tau_h = tau_h / phi_h ;
    return 0; }
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
  ...
  h = h0;
 {
   phi_h = pow( q10 , ( ( celsius - 24.0 ) / 10.0 ) ) ;
   h = 0.0 ;
   }
