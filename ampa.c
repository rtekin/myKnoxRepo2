...
#define Rinf Rinf_AMPA_S
 double Rinf = 0;
#define deadtime deadtime_AMPA_S
 double deadtime = 1;
 ...
 "deadtime_AMPA_S", &deadtime_AMPA_S,
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 5;
 	...
 	ivoc_help("help ?1 AMPA_S d:/Dropbox/neuro_sim_lab/netpyne_scripts/my_knox2/ampa.mod\n");
...
{    _p = _pnt->_prop->param; _ppvar = _pnt->_prop->dparam;
 ...
 _tsav = t;   if (_lflag == 1. ) {*(_tqitem) = 0;}
 {
   if ( _lflag  == 0.0 ) {
     if (  ! _args[1] ) {
       synon = synon + _args[0] ;
       _args[4] = _args[2] * exp ( - Beta * ( t - dt - _args[3] ) ) ;
   ...
   if ( _lflag  == 1.0 ) {
     synon = synon - _args[0] ;
     _args[4] = _args[0] * Rinf + ( _args[2] - _args[0] * Rinf ) * exp ( - ( t - dt - _args[3] ) / Rtau ) ;
     _args[2] = _args[0] * Rinf + ( _args[2] - _args[0] * Rinf ) * exp ( - ( t - _args[3] ) / Rtau ) ;
     Ron = Ron - _args[2] ;
     Roff = Roff + _args[4] ;
     _args[3] = t ;
     net_send ( _tqitem, _args, _pnt, t +  deadtime , 2.0 ) ;
     }
   if ( _lflag  == 2.0 ) {
     _args[1] = 0.0 ;
     }
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  release();
 if(error){fprintf(stderr,"at line 105 in file ampa.mod:\n	SOLVE release METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
